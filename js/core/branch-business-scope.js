...
(function(){
  ...
  window.renderFinance=function(){
    syncSelectors();baseRenderFinance();const scope=allowedScope(scopes.finance),m=$('financeMonth')?.value||'2026-07',d=financeData(m);
  ...
  window.renderSettlement=function(){
    ...
    $('studentSettleRows').innerHTML=sr.map(x=>{const parent=String(x.s.parent||'').trim().toLocaleLowerCase(),familyIds=(db.students||[]).filter(s=>!s.campSeason&&(s.status||'active')!=='inactive'&&(parent?String(s.parent||'').trim().toLocaleLowerCase()===parent:s.id===x.s.id)).map(s=>s.id);return`<tr><td><b>${esc(x.s.name)}</b>${x.campAmount?`<br><span class="small">含 Summer Camp ${money(x.campAmount)}</span>`:''}</td><td>${esc(x.s.parent)}</td><td>${x.total}</td><td>${x.charged} 堂／${fmtHours(x.h)} hr</td><td>${x.abs}</td><td>${x.rate.toFixed(1)}%</td><td>${money(x.amount)}</td><td><button class="btn line-billing-btn" onclick="copyStudentLineBilling('${x.s.id}','${$('settleMonth').value}','${scope}','${encodeURIComponent(familyIds.join(','))}')">複製家庭 LINE</button></td></tr>`}).join('')||'<tr><td colspan="8" class="small">此校區本月沒有學生收入資料。</td></tr>';
