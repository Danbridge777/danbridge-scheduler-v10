...
(function(){
  ...
  window.renderSettlement=function(){
    ...
    $('teacherPayCards').innerHTML=tr.map(x=>`<div class="teacher-pay-card"><div class="teacher-pay-head"><div class="teacher-pay-avatar">${esc((x.t.name||'?').trim().charAt(0).toUpperCase())}</div><div class="teacher-pay-identity"><h4 title="${esc(x.t.name)}">${esc(x.t.name)}</h4><div class="teacher-pay-role">教師工時與薪資</div></div></div><div class="teacher-work-summary">${x.companyWide?`固定工作日：${esc(workDayNames(x.t.workDays))}｜每週最低：${fmtHours(x.t.minWeeklyHours)} hr`:`${esc(scopeLabel(scope))} KPI｜最低工時與差額只在「全部校區」合併計算`}</div><div class="teacher-pay-grid"><div class="teacher-pay-stat">${x.companyWide?'本月最低':'校區堂數'}<b>${x.companyWide?fmtHours(x.expected)+' hr':x.count+' 堂'}</b></div><div class="teacher-pay-stat">實際上課<b>${fmtHours(x.h)} hr</b></div><div class="teacher-pay-stat">${x.companyWide?'多／少':'校區占比'}<b class="${x.companyWide?diffClass(x.diff):''}">${x.companyWide?diffText(x.diff):(totalHours?((x.h/totalHours)*100).toFixed(1)+'%':'0%')}</b></div><div class="teacher-pay-stat">應付薪資<b>${money(x.amount)}</b></div></div>${x.companyWide&&x.branches.length?`<div class="branch-breakdown cards">${x.branches.map(b=>`<span><b>${esc(scopeLabel(b.branchId))}</b>${fmtHours(b.h)} hr／${money(b.amount)}</span>`).join('')}</div>`:''}${x.companyWide?`<div class="weekly-detail"><div class="weekly-row head"><span>週別</span><span>最低</span><span>實際</span><span>差額</span></div>${x.weeks.map(w=>`<div class="weekly-row"><span>${w.from.slice(5)}～${w.to.slice(5)}</span><span>${fmtHours(w.expected)}</span><span>${fmtHours(w.actual)}</span><span class="${diffClass(w.diff)}">${diffText(w.diff)}</span></div>`).join('')}</div>`:''}</div>`).join('')||'<span class="small">目前尚未建立老師。</span>';
