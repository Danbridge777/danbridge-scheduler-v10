...
function normalizeBranchData(x){
 ...
 (x.summerCampRegistrations||[]).forEach(r=>{const st=(x.students||[]).find(a=>a.id===r.studentId),lessonBranch=(x.lessons||[]).find(l=>l.studentId===r.studentId&&l.branchId&&l.branchId!=='unassigned')?.branchId,branchId=r.branchId||(st?.branchIds||[])[0]||lessonBranch||'unassigned',dates=[...new Set(Array.isArray(r.dates)?r.dates:[])].filter(d=>typeof d==='string'&&(!r.month||d.startsWith(r.month+'-'))).sort(),month=r.month||(dates[0]||'').slice(0,7),key=`${r.studentId||''}|${month}`;if(!r.studentId||!month)return;const prev=registrationMap.get(key),current={...r,id:r.id||uid(),branchId,month,dates,dailyRate:+r.dailyRate||0,totalFee:dates.length*(+r.dailyRate||0),updatedAt:r.updatedAt||new Date(0).toISOString()};if(!prev){registrationMap.set(key,current);return}const newer=String(current.updatedAt)>=String(prev.updatedAt)?current:prev,mergedDates=[...new Set([...(prev.dates||[]),...(current.dates||[])])].sort(),dailyRate=+newer.dailyRate||+prev.dailyRate||+current.dailyRate||0;registrationMap.set(key,{...newer,id:prev.id||current.id,branchId:newer.branchId&&newer.branchId!=='unassigned'?newer.branchId:(prev.branchId||current.branchId||'unassigned'),dates:mergedDates,dailyRate,totalFee:mergedDates.length*dailyRate})});
