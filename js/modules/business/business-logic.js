...
/* V17.24 — company timetable revenue is counted per teacher schedule entry.
 * A lesson assigned to three teachers contributes three full revenue entries.
 * Collection confirmation, payment and report status remain display-only metadata. */
function timetableRevenueCharge(l){
  if(!l||l.isDraft)return 0;
  const teacherCount=Math.max(1,lessonTeacherIds(l).length);
  return lessonCharge(l)*teacherCount;
}
 ...
 * calculates every scheduled lesson as the student's hourly rate multiplied by its scheduled duration.
 * Payment, collection confirmation, report and lesson status never change timetable revenue. */
...
function financeData(m){const lessons=db.lessons.filter(l=>l.date.startsWith(m));const revenue=lessons.reduce((a,l)=>a+timetableRevenueCharge(l),0);const fixed=(db.fixedExpenses||[]).filter(x=>fixedExpenseApplies(x,m));const one=(db.oneTimeExpenses||[]).filter(x=>x.month===m);const fixedTotal=fixed.reduce((a,x)=>a+(+x.amount||0),0);const oneTimeTotal=one.reduce((a,x)=>a+(+x.amount||0),0);const payrollRows=db.teachers.map(t=>{const paid=teacherPaidLessons(t,m),payroll=calculateTeacherPayroll(t,m,paid);return{teacher:t,h:payroll.actualHours,amount:payroll.amount,payroll}}).filter(x=>x.h||x.amount);const payroll=payrollRows.reduce((a,x)=>a+x.amount,0);const totalExpenses=fixedTotal+oneTimeTotal+payroll;return{m,revenue,fixed,one,fixedTotal,oneTimeTotal,payrollRows,payroll,totalExpenses,profit:revenue-totalExpenses}}
...
function settleData(){const m=$('settleMonth').value||monthNow(),ls=db.lessons.filter(l=>!l.isDraft&&l.date.startsWith(m));const sr=db.students.map(s=>{const x=ls.filter(l=>l.studentId===s.id),abs=x.filter(l=>['學生請假','老師請假','取消','停課'].includes(l.status));return{s,total:x.length,charged:x.length,h:x.reduce((a,l)=>a+hours(l.start,l.end),0),abs:abs.length,rate:x.length?abs.length/x.length*100:0,amount:x.reduce((a,l)=>a+timetableRevenueCharge(l),0)}}).filter(x=>x.total);const tr=db.teachers.map(t=>{const paid=teacherPaidLessons(t,m),payroll=calculateTeacherPayroll(t,m,paid),weeks=teacherWeekBreakdown(t,m);return{t,count:paid.length,h:payroll.actualHours,expected:payroll.expectedHours,diff:payroll.diff,weeks,amount:payroll.amount,revenue:teacherCompanyRevenue(t,m,ls),payroll}});return{sr,tr}}
