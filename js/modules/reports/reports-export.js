...
function exportVisibleAppleCalendar(){const range=visibleCalendarRange(),filters=calendarFilterState(),lessons=db.lessons.filter(l=>!l.isDraft&&l.date>=range.start&&l.date<=range.end&&lessonMatchesCalendar(l,filters)).sort((a,b)=>a.date.localeCompare(b.date)||a.start.localeCompare(b.start));if(!lessons.length)return alert('目前畫面沒有可匯入 Apple 行事曆的正式課程。');const mode=$('calendarMode')?.value==='week'?'本週':'本月',fileName=`Danbridge-${range.start}-${range.end}.ics`;downloadAppleCalendar(appleCalendarContent(lessons,{showDate:true}),fileName);toast(`已準備 ${lessons.length} 堂${mode}課程，請在 Apple 行事曆確認加入`)}
